    ...
    try {
      ...
      if (managers.length > 0) {
        managersText = managers.map((manager, index) =>
          `${index + 1}. ${manager.full_name} (${manager.username ? `@${manager.username.replace('@', '')}` : 'N/A'})\n` +
      ...
      if (baristas.length > 0) {
        baristasText = baristas.map((barista, index) =>
          `${index + 1}. ${barista.full_name} (${barista.username ? `@${barista.username.replace('@', '')}` : 'N/A'})\n` +
  ...
  private async initializeDefaultSettings(defaults: any): Promise<void> {
    try {
      ...
      const settingsToInsert = [
        ...
        { key: 'birthdayBonus', value: '10', type: 'number', desc: 'Бонус в день рождения' },
    ...
    try {
      ...
      if (managers.length === 0) {
        ...
        managers.forEach((manager, index) => {
          ...
          text += `   ${manager.username ? `@${manager.username.replace('@', '')}` : 'без username'}\n`;
    ...
    try {
      ...
      if (baristas.length === 0) {
        ...
        baristas.forEach((barista, index) => {
          ...
          text += `   ${barista.username ? `@${barista.username.replace('@', '')}` : 'без username'}\n`;
